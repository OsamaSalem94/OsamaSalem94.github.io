...
const App = () => {
  const [data, setData] = useState<IAllProjectsData>({
    projects: [],
    otherProjects: [],
  });
  ...
  const fetchData = () => {
    fetch(
      "https://my-portfolio-2dbb1-default-rtdb.europe-west1.firebasedatabase.app/.json"
    )
      .then((res) => res.json())
  ...
  return (
    <div className="App">
      <MainContext.Provider
        value={{ projects: data.projects, otherProjects: data.otherProjects }}
      >
        {isLoading ? (
          <Loader
            finishAnimating={() => {
